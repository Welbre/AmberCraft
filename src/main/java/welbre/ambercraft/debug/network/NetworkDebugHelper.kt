...
class NetworkDebugHelper(entity: ModulesHolder): Serializable
{
    ...
    companion object{
        fun parser(buf: FriendlyByteBuf): NetworkDebugHelper {
            try {
                val data: String = buf.readUtf();
                val stream = ByteArrayInputStream(Base64.getDecoder().decode(data))
                val inputSteam = ObjectInputStream(stream)
                val result:NetworkDebugHelper = inputSteam.readObject() as NetworkDebugHelper;
                return result;
            } catch (crossinline e: Exception) {
                throw e;
            }
        }
    }
